...
function Pool(config) {
  ...
  this._props = (function (config) {
    ...
    // if property does not exist it returns 'null' in the new java version, so we check for both
    // null and undefined to support older versions as well

    if (config.user) {
      var user = properties.getPropertySync('user');
    ...
    if (config.password) {
      var password = properties.getPropertySync('password');
...
var connStatus = function(pool) {
  return pool.map(function(connobj) {
...
Pool.prototype._addConnectionsOnInitialize = function(callback){
  ...
  asyncjs.times(self._minpoolsize, function(n, next){
    ...
    if (err) {
      ...
      self._pool = self._pool.concat(conns);
...
Pool.prototype.release = function(conn, callback) {
  ...
  if (typeof conn === 'object') {
    var uuid = conn.uuid;
    self._reserved = self._reserved.filter(function(conn) {
